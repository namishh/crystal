...
function prompt:start()
  ...
  if self.reset_on_stop == true then self.text = ""
    self.command = ""
  end
  ...
  self._private.grabber = keygrabber.run(function(modifiers, key, event)
    ...
    if mod.Control then
      ...
      if key == "v" then
        ...
        if selection then
          ...
          self.command = self.command:sub(1, self._private_cur_pos - 1) ..
              selection .. self.command:sub(self._private_cur_pos)
      ...
      elseif key == "h" then
        if self._private_cur_pos > 1 then
          ...
          self.command = self.command:sub(1, self._private_cur_pos - 2 - offset) ..
    ...
    elseif mod.Mod1 or mod.Mod3 then
      ...
      elseif key == "d" then
        self.command = self.command:sub(1, self._private_cur_pos - 1) ..
            self.command:sub(cword_end(self.command, self._private_cur_pos))
      ...
      if self.completion_callback then
        if key == "Tab" or key == "ISO_Left_Tab" then
          ...
          local matches
          self.command, self._private_cur_pos, matches = self.completion_callback(command_before_comp,
            cur_pos_before_comp, ncomp)
          ncomp = ncomp + 1
          key = ""
